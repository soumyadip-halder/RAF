...
export const getActionTypes = (type) => {
  let url = `${BASE_URL}${GET_CONFIGS}`
  url = url.replace('{configType}', type)
  // let reqBody = `${JSON.stringify(req)}`
  return serviceRequest(url, 'GET', undefined)
}

export const getRangeResetPerEvent = (eventId) => {
  let url = `${BASE_URL}${GET_RANGERESET_PEREVENT}`
